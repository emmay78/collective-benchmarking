...
def times_intra_node(time_mat):
    ...
    for pair in prod:
        if pair[0] != pair[1]:
            if (pair[0] < world_size/num_nodes and pair[1] < world_size/num_nodes) or (pair[0] >= world_size/num_nodes and pair[1] >= world_size/num_nodes):
...
def times_inter_node(time_mat):
    ...
    for pair in prod:
        if pair[0] != pair[1]:
            if not ((pair[0] < world_size/num_nodes and pair[1] < world_size/num_nodes) or (pair[0] >= world_size/num_nodes and pair[1] >= world_size/num_nodes)):
...
# Read send times
df = pd.read_csv(f"{bw_results_dir}/bw_0_send.data", header=None).rename(columns={0:"data_size"})
df["source_rank"] = 0
df = df.sort_values(by=["data_size", "source_rank"])
for rank in range(1, world_size):
    df2 = pd.read_csv(f"{bw_results_dir}/bw_{rank}_send.data", header=None).rename(columns={0:"data_size"})
...
inter_max_times = df.groupby(["data_size"], group_keys=False).apply(lambda x: np.max(times_inter_node(x.drop(["data_size"], axis=1).to_numpy())))
...
intra_max_times = df.groupby(["data_size"], group_keys=False).apply(lambda x: np.max(times_intra_node(x.drop(["data_size"], axis=1).to_numpy())))
